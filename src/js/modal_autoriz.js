...
const firebaseConfig = {
  ...
  databaseURL:
    'https://authfun-db86b-default-rtdb.europe-west1.firebasedatabase.app/',
};
...
const mobileSignInREf = document.querySelector(
  '.header-user-modal.js-modal-autoriz-open'
);
...
const headerDeskLogOut = document.querySelector(
  '.header-user.js-modal-autoriz-open.js-logout'
);
...
async function loginInAcc(e) {
  ...
  try {
    const userCregential = await signInWithEmailAndPassword(
      auth,
      loginEmail,
      loginPassword
    );
...
async function createNewAcc(e) {
  ...
  try {
    ...
    const userCregential = await createUserWithEmailAndPassword(
      auth,
      loginEmail,
      loginPassword
    );
    ...
    formRegRef.reset();
    formLogRef.reset();
    Notiflix.Notify.success('You successfuly create account!');
  } catch (error) {
    console.log(error);
